  ...
  reducers: {
    add_cart_product: (state, { payload }) => {
      ...
      if (!isExist) {
        ...
        notifySuccess(`${state.orderQuantity} ${payload.title} añadido al carrito`);
      } else {
        state.cart_products.map((item) => {
          if (item._id === payload._id) {
            if (item.quantity >= item.orderQuantity + state.orderQuantity) {
              ...
              notifySuccess(`${state.orderQuantity} ${item.title} añadido al carrito`);
            } else {
    ...
    clearCart:(state) => {
      const isClearCart = window.confirm('¿Estás seguro de que quieres eliminar todos los artículos?');
      if(isClearCart){
        state.cart_products = []
      }
      setLocalStorage("cart_products", state.cart_products);
